...
```{r focalClade}
(Halobacterium <- focalClade(Tibble, "Halobacterium_noricense", "Halobacterium_salinarum", "blue", "Halobacterium"))
...
```{r focalCladeList}
Haloferax <- focalClade(Tibble, "Haloferax_mediterranei", "Haloferax_volcanii", "green3", "Haloferax")
...
MRCA_2D_plot( exDataFrame, clades
            , x = "percent_difference_local"
            , y = "index_avg_strandDiscord")
```

In this example, the plots are not dense but in real datasets they can contain
tens of thousands of points.  Remember that we have two values per pair of
species (A vs B and B vs A).  To simplify the plot by averaging the two
alignment replicates, use `averageResults()` like below.

```{r MRCA_plot_sym}
exDataFrameSym <- averageResults(exDataFrame)
MRCA_2D_plot(exDataFrameSym, clades)
```

In ellipse plots, all the pairs of species are displayed and grouped by MRCA.
These groups are colored by focal clades.

```{r ellipse_plot}
ellipsePlot( exDataFrameSym
           , x = "percent_difference_local"
           , y = "index_avg_strandDiscord")
```
