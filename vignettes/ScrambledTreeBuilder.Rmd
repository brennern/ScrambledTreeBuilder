...
Finally, we can use the 'tibbleWithValue' and 'tibbleWithMultipleValues' dataset to build the tree. In our case, we will generate a tree built based on percent identity values with strand randomisation scores labeled on all nodes using 'tibbleWithValue'. 'tibbleWithMultipleValues' will allow us to plot both strand randomisation index scores and percent identity scores on the tree.
