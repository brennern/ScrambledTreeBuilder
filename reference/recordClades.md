...
Add clade information to tables, for quality checks and use in some
plots.

## Usage

``` r
recordClades(df, clades)
```
...
Returns the table, plus a `customClade` column if it was a taxon table,
or a `focalClade` and `focalColor` columns if it was a results table.

## Note

As MRCA information is needed; the function will run
