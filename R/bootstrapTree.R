...
#'
#' @return A `list` with:
#'  - `tree`: Reference tree (`phylo`) built from `D` with UPGMA. Node labels contain bootstrap %.
#'  - `bootstrap_trees`: List of replicate trees (`phylo`).
#'  - `support`: Numeric vector of bootstrap percentages per internal node (aligned to `tree$node.label`).
#'  - `call`: Matched call.
#'
#' @author Anika Mittal
#'
